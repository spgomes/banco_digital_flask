...
class ContaPersistence():
    ...
    def deposito_saldo(self, id, valorDeposito):
        self.db.execute('UPDATE Conta SET Saldo = Saldo + ? WHERE id = ?', [valorDeposito, id])
        return True
    
    def retirada_saldo(self, id, valorRetirada):
        self.db.execute('UPDATE Conta SET Saldo = Saldo - ? WHERE id = ?', [valorRetirada, id])
        return True

    def save_deposito(self, historico):
        self.db.execute('INSERT INTO Historico (Data, ValorEntrada, Conta_id) VALUES (?,?,?)', 
                        [
                            historico['Data'],
                            historico['ValorEntrada'],
                            historico['Conta_id']
                            ])
        return True
    
    def save_retirada(self, historico):
        self.db.execute('INSERT INTO Historico (Data, ValorSaida, Conta_id) VALUES (?,?,?)',
                    [
                            historico['Data'],
    ...
    def save_transferencia(self, historico):
        self.db.execute('INSERT INTO Historico (Data, ValorSaida, Conta_id, Conta_destino) VALUES (?,?,?,?)',
                    [
                            historico['Data'],
                            historico['ValorSaida'],
                            historico['Conta_id'],
